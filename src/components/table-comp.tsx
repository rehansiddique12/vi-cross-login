    ...
    <div className="bg-white rounded-2xl h-1/2 w-full p-4 justify-self-center mb-4">
      ...
      <div className="flex justify-between items-center mb-2">
        <button className="flex items-center gap-2 bg-Blue hover:bg-White hover:text-Blue hover:border text-white px-4 py-2 rounded-md">
          ...
          <span className="border border-white rounded-md w-5 h-5 flex items-center justify-center text-sm">
            +
          </span>
        </button>
        ...
        <div className="flex items-center gap-4">
          <div className="flex">
            <button
              className={`px-4 py-1 ${
                activeView === "day"
                  ? "bg-Blue hover:bg-White hover:text-Blue hover:border text-white rounded-md"
                  : "text-gray-600"
              }`}
      ...
      <div className="grid grid-cols-[auto_repeat(7,1fr)] border border-gray-200 rounded-md">
        ...
        {days.map((day) => (
          <div
            key={day}
            className="p-2 text-center font-medium border-b border-r border-gray-200"
          >
            {day}
          </div>
        ))}
        ...
        {timeSlots.map((time) => (
          <React.Fragment key={time}>
            {/* Time label */}
            <div className="p-2 text-right pr-4 border-b border-r border-gray-200 text-gray-600">
              {time}
            </div>
            ...
            {days.map((day) => {
              ...
              return (
                <div
                  key={`${day}-${time}`}
                  className="border-b border-r border-gray-200 relative h-[50px]"
                >
                  {isStartTime && event && (
                    <div
                      className={`absolute inset-1 pl-2 ${getEventColor(
                        event.type
                      )}`}
                    >
